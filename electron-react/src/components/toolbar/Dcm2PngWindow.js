...
function Dcm2PngWindow(props){
    ...
    function _onChange(e) {
        ...
        if(e.target.id === 'low_clip'){
          props.setState({ showDialog: props.state.showDialog, low_clip: e.target.value, high_clip: props.state.high_clip, files: props.state.files});
        }
        else if(e.target.id === 'high_clip') {
          props.setState({ showDialog: props.state.showDialog, low_clip: props.state.low_clip, high_clip: e.target.value, files: props.state.files});
        }
     }
        ...
        <DialogModal>
                <div className="dialog-wrapper">
                    ...
                    <form onSubmit={_onSubmit}>
                        {props.preview!=="" && <div className="previewDiv"> <img className="previewSlice" src={props.preview} alt="Preview" /> </div>}
                        <div className="previewDiv2">
                        LOW: <input type="text" id="low_clip" value={props.state.low_clip} onChange={_onChange} /> {" "}
                        HIGH: <input type="text" id="high_clip" value={props.state.high_clip} onChange={_onChange} />{" "}
                        </div>
                        <div className="previewDiv2">
